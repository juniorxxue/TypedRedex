...
-- Flip polarity
flipPolar :: (Redex rel) => L Polar rel -> L Polar rel -> Relation rel
...
isUvar :: (Redex rel) => L Env rel -> L Nat rel -> Relation rel
...
isEvar :: (Redex rel) => L Env rel -> L Nat rel -> Relation rel
...
findSol :: (Redex rel) => L Env rel -> L Nat rel -> L Ty rel -> Relation rel
...
inst :: (Redex rel) => L Env rel -> L Nat rel -> L Ty rel -> L Env rel -> Relation rel
...
ssub :: (Redex rel) => L Env rel -> L Ty rel -> L Polar rel -> L Ty rel -> Relation rel
...
main = do
  ...
  let synth = runSubstRedex $ fresh $ \e -> do
