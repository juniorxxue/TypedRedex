...
fresh :: (Redex rel, LogicType a) => (LTerm a rel -> rel s) -> rel s
fresh f = fresh_ FreshVar $ f . Free

-- | Allocate two fresh variables.
fresh2 :: (Redex rel, LogicType a, LogicType b) => (LTerm a rel -> LTerm b rel -> rel s) -> rel s
fresh2 f = fresh $ \a -> fresh $ \b -> f a b

-- | Allocate three fresh variables.
fresh3 :: (Redex rel, LogicType a, LogicType b, LogicType c) => (LTerm a rel -> LTerm b rel -> LTerm c rel -> rel s) -> rel s
fresh3 f = fresh $ \a -> fresh $ \b -> fresh $ \c -> f a b c

-- | Allocate four fresh variables.
fresh4 :: (Redex rel, LogicType a, LogicType b, LogicType c, LogicType d) => (LTerm a rel -> LTerm b rel -> LTerm c rel -> LTerm d rel -> rel s) -> rel s
fresh4 f = fresh $ \a -> fresh $ \b -> fresh $ \c -> fresh $ \d -> f a b c d

-- | Allocate five fresh variables.
fresh5 :: (Redex rel, LogicType a, LogicType b, LogicType c, LogicType d, LogicType e) => (LTerm a rel -> LTerm b rel -> LTerm c rel -> LTerm d rel -> LTerm e rel -> rel s) -> rel s
