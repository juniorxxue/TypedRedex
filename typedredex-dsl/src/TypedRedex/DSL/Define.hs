...
import TypedRedex.Logic (Redex(..), CapturedTerm(..), Logic(..), LogicType(..))
...
data LTermList rel (ts :: [Type]) where
  TNil  :: LTermList rel '[]
  (:>)  :: (LogicType t, Typeable t) => LTerm t rel -> LTermList rel ts -> LTermList rel (t ': ts)

infixr 5 :>

--------------------------------------------------------------------------------
-- Constraint family: all types in list must be LogicType and Typeable
--------------------------------------------------------------------------------

-- | Constraint that all types in the list are LogicType and Typeable instances.
type family AllLogicType (ts :: [Type]) :: Constraint where
  AllLogicType '[]       = ()
  AllLogicType (t ': ts) = (LogicType t, Typeable t, AllLogicType ts)
...
instance (LogicType t, Typeable t, CaptureTermsList rel ts) => CaptureTermsList rel (t ': ts) where
