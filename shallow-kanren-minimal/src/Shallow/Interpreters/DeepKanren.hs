...
module Shallow.Interpreters.DeepKanren
  ( DeepKanren
  , Goal(..)
  , runDeep
  , prettyGoal
...
-- Deep Embedding AST
--
-- This captures the structure of a Kanren relation as data.
--------------------------------------------------------------------------------
...
--------------------------------------------------------------------------------
-- Deep Kanren Interpreter
--
-- Builds a Goal AST instead of solving.
--------------------------------------------------------------------------------

newtype DeepKanren a = DeepKanren (State DeepState a)
  deriving (Functor, Applicative, Monad, MonadState DeepState)

instance Alternative DeepKanren where
  empty = DeepKanren $ pure (error "DeepKanren: empty")
  a <|> b = do
...
instance Kanren DeepKanren where
  newtype KVar DeepKanren t = DVar Int
    deriving (Functor)
  ...
  call_ _ (Relation _ body) = do
    depth <- gets dsDepth
    if depth < maxDepth
      then do
        -- Increment depth and run the body to capture onRelationCall
        modify $ \s -> s { dsDepth = depth + 1 }
        body
        modify $ \s -> s { dsDepth = depth }  -- restore depth
      else
        -- At max depth, don't expand further (onRelationCall already recorded)
        pure ()

  displayVar (DVar n) = freshName n

  -- This is the key: capture relation calls with their arguments
  onRelationCall name args = do
    recordGoal $ GCall name args

instance EqVar DeepKanren where
  varEq (DVar a) (DVar b) = a == b

--------------------------------------------------------------------------------
-- Pretty-printing Logic terms (local version for DeepKanren)
--------------------------------------------------------------------------------

prettyL :: LogicType a => Logic a (KVar DeepKanren) -> String
prettyL (Free (DVar n)) = freshName n
prettyL (Ground r) = prettyReified r

prettyReified :: LogicType a => Reified a (KVar DeepKanren) -> String
prettyReified r =
  let (con, fields) = quote r
  in formatCon (name con) (map prettyField fields)

prettyField :: Field a (KVar DeepKanren) -> String
prettyField (Field _ logic) = prettyLogicAny logic

prettyLogicAny :: LogicType t => Logic t (KVar DeepKanren) -> String
...
-- | Run a DeepKanren computation and extract the Goal AST
runDeep :: DeepKanren () -> Goal
runDeep (DeepKanren m) =
...
formatAsRule ruleName goal =
  let (prems, lhs, rhs) = extractRule goal
      -- Renumber variables to start from 0
      allVars = collectVars (prems ++ [lhs, rhs])
      renumberMap = zip allVars [0..]
...
deepVar :: Int -> Logic a (KVar DeepKanren)
